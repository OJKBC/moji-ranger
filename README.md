...
- **反応速度**: `continuous=true`＋`interimResults=true` で認識を回しっぱなしにし、**途中経過(interim)で
  一致した時点で final を待たず即・正解**（発話→演出まで体感1秒以内）。**不正解の確定は final のみ**
  （interim の誤認識では減点しない＝速いのは正解側、誤答側は慎重に）。
