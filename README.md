...
## ゲームの流れ（進む → 対峙 → 浄化 → また進む）

1. **進む**: カメラが街を自動前進（移動中も自由にビームを撃てる・無罰）
2. **対峙**: 敵エリアでなめらかに減速し、もやもやモンスターと正面から対峙。浄化メーターが出る
3. **浄化**: 敵の周りに文字バブルが並ぶ（**不透明・最前面で常にくっきり**）。正しい文字を撃つたびに
   もやが1段晴れて敵が明るくなり、規定回数で笑顔になって空へ帰る。次に狙う文字は学習システム
   （`src/learning/picker.ts`・簡易間隔反復）が選ぶ
4. ルートの最後まで進んだらゴール
...
### 2.5D 対峙ステージ（renderer: '2.5d'）

`segments`（前進距離と対峙の並び）と `encounters`（対峙の中身）を書くだけでルートが増える:

```ts
segments: [
  { type: 'ride', distance: 780 },
  { type: 'encounter', encounterId: 'park-haze-1' },
  { type: 'ride', distance: 620 },
],
encounters: [
  {
    id: 'park-haze-1',
    layout: 'A_single_boss',   // B_enemy_per_choice はフェーズ2
    purifySteps: 3,            // 完全浄化に必要な正解回数
    choiceCount: 5,            // 選択肢の数
    letterPool: ['あ'],        // 学習システムがここから次の文字を選ぶ
    distractorPool: [{ label: 'お', kind: 'hiragana' }, /* ... */],
  },
],
```
