...
- ひらがな（難易度が上がると単語）を **声に出して読む** ステージ。判定はブラウザ標準の
  Web Speech API（`SpeechRecognition`, `ja-JP`）。実装は `src/ReadingStage.tsx` と `src/speech.ts`。
...
- **幼児向けの寛容判定**（`judgeReading`）: 一致・一部一致・1文字違いは正解。**認識できなかったときは
  誤答にせず「もういちど いってみよう！」でやり直し**（ハートは減らない）。明確な違いのみ誤答＝ハート-1。
- **お手本**ボタンで読み方を再生できる（補助あり正解は `assistedCorrect` に記録し熟達度に加算しない）。
